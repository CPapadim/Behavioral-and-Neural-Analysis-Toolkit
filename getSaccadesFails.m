function [saccadeAngles, saccadeRadii, saccadeX, saccadeY] = getSaccadesFails(filename,stack_opts)
...
%		saccadeY - saccade y-coordinate
	...
	grabtargs=['grab ' stack_opts ' -aB1 -ax -o8 -i-50:50 ' filename];
	...
	if(isstruct(target))
		...
		grabsaccades=['grab ' stack_opts ' -aB1 -ax -o9 -i-50:50 ' filename];
		[status,result]=system(grabsaccades);
		system(['mv -f BehavIndi* BehavIndi' splitfile{6}(2:end)]);
		file_saccades=['BehavIndi' splitfile{6}(2:end)];
