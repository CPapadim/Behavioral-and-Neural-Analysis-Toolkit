...
%Parameters when running as script - Comment out when using as function
filelist='/home/cpapadim/m-files/Data_Analysis/NeuronScripts/cellLists/SaccMem';
perSessionOrPop='Session';
rawOrSmooth='Smooth';
stack_opts_C='-Xs212:213';
stack_opts_P='-Xs206:207';
targloc_num=1; % 1 - first target location, 2 - second target location
...
[filename monkey date unit pref_dir specified_null_dir xcoord ycoord zcoord area]=readCellList(filelist);
        ...
        alltrialnewtarganglesC=[];
        alltrialnewtarganglesP=[];
        alltrialtarganglesC=[];
        alltrialtarganglesP=[];
        alltrialsaccanglesC=[];
        alltrialsaccanglesP=[];
	alltrialsaccResErrTh=[];
		...
		splitfile=regexp(filename{j},'/','split');
			...
			%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			[targetTh, targetR, targetX, targetY, targetThNew, targetRNew, targetXNew, targetYNew] = getTargetsSaccMem(filename{j},'');
			[saccadeTh, saccadeR, saccadeX, saccadeY] = getSaccadesSaccMem(filename{j},'');
			...
			saccadeXP=saccadeX(keepidxPrev);
			saccadeYP=saccadeY(keepidxPrev);
			saccadeXC=saccadeX(keepidxCurr);
			saccadeYC=saccadeY(keepidxCurr);
			
			prevnewtargangles=targetThNew(keepidxPrev)';
			currnewtargangles=targetThNew(keepidxCurr)';
			...
			alltrialnewtarganglesP=[alltrialnewtarganglesP; prevnewtargangles];
			alltrialnewtarganglesC=[alltrialnewtarganglesC; currnewtargangles];
	...
	elseif(targloc_num == 2)
        	relativeanglediff=alltrialnewtarganglesP-alltrialtarganglesC;
	end
        %relativeanglediff=alltrialsaccanglesP-alltrialtarganglesC;
