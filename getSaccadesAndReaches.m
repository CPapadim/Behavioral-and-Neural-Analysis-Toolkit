...
%		saccadeY - saccade y-coordinate
	...
	system(['rm -f BehavMean*']);
	grabsaccades=['grab ' stack_opts ' -aB1 -o9 -i-50:50 ' filename];
	[status,result]=system(grabsaccades);
	system(['mv -f BehavMean* BehavMean_' splitfile{6}(2:end)]);
	file_saccades=['BehavMean_' splitfile{6}(2:end)];
	...
	system(['rm -f ' file_saccades]);
        ...
        reachX=(saccade.data(:,6)-fixationX);
       	reachY=(saccade.data(:,7)-fixationY);
