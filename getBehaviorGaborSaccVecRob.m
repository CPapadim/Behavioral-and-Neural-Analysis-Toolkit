function [gaborFit,gofGabor, relativeGaborAngles,residualError] = getBehaviorGaborSaccVecRob(filelist,perSessionOrPop,rawOrSmooth,stack_opts_C,stack_opts_P);
...
[filename monkey date unit pref_dir specified_null_dir xcoord ycoord zcoord area]=readCellList(filelist);
	...
	for j=1:length(filename)
		...
		splitfile=regexp(filename{j},'/','split');
			...
			[saccadeTh, saccadeR, saccadeX, saccadeY, targetTh, targetR, targetX, targetY] = getSaccadeVectorsRob(filename{j},'');
